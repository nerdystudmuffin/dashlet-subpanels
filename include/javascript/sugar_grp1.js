...
function addToValidateLessThan(formname,name,type,required,msg,max){addToValidate(formname,name,type,required,msg);validate[formname][validate[formname].length-1][jstypeIndex]='less';validate[formname][validate[formname].length-1][maxIndex]=max;}
...
break;case'less':value=parseInt(trim(form[validate[formname][i][nameIndex]].value));maximum=parseInt(validate[formname][i][maxIndex]);if(typeof maximum!='undefined'){if(value>maximum)
add_error_style(formname,validate[formname][i][nameIndex],validate[formname][i][msgIndex]+" "+SUGAR.language.get('app_strings','MSG_IS_MORE_THAN')+' '+maximum);}

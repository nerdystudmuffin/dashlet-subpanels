	...
	switch($sugar_config['dbconfig']['db_type']){
		...
		case 'mssql':
			...
			if(ini_get('magic_quotes_gpc') == 1) {
				...
				$ret['mssqlStatus'] = "<b><span class=stop>{$installer_mod_strings['ERR_CHECKSYS_MSSQL_MQGPC']}</span></b>";
				$ret['error_found'] = true;
			}
	        break;
	    case 'oci8':
...
function prepSystemForUpgrade() {
    ...
    $upgrade_progress_dir = getcwd().'/'.$GLOBALS['sugar_config']['cache_dir'].'upgrades/temp';

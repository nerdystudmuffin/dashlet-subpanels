...
function getDiscountCheckbox($focus, $field='' , $value='' , $view='DetailView'){
    if($view == 'EditView'){
        if($focus->fetched_row['discount_select']== '%'){
            $html = '<input id="discount_select" name="discount_select" value="$" style="display: none;"/>
             <input id="discount_select" class="checkbox"
             type="checkbox" name="discount_select" value="%"
             checked=""/>';
        ...
        $html = $focus->fetched_row['discount_select'];
    }
return $html;
}

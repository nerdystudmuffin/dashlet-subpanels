...
<?php
    if( $install_type == "module" ){
        ...
        if( sizeof( $files_to_handle ) > 0 ){
            echo '<div style="text-align: right; cursor: hand; cursor: pointer; text-decoration: underline;" onclick=\'this.style.display="none"; toggleDisplay("more");\' id="all_text">Show Details <img src="'.SugarThemeRegistry::current()->getImageURL('advanced_search.gif').'"></div><div id=\'more\' style=\'display: none\'>
           	     <div style="text-align: right; cursor: hand; cursor: pointer; text-decoration: underline;" onclick=\'document.getElementById("all_text").style.display=""; toggleDisplay("more");\'>Hide Details <img name="options" src="'.SugarThemeRegistry::current()->getImageURL('basic_search.gif').'"></div><br>';

 	...
 	function constructAjax ()
 	{
        ...
        $layoutView = 'layouts' ;
 		...
 		if ($this->fromModuleBuilder)
 		{
 			...
 			if ( $layoutLabel == 'LBL_LAYOUTS' ) $ajax->addCrumb ( translate ( 'LBL_SEARCH_FORMS', 'ModuleBuilder' ), 'ModuleBuilder.getContent("module=ModuleBuilder&MB=true&action=wizard&search=1&view_module=' .$this->editModule . '&view_package=' . $_REQUEST [ 'view_package' ] . '")'  ) ;
 			$ajax->addCrumb ( translate ( $searchLabel, 'ModuleBuilder' ), 'ModuleBuilder.getContent("module=ModuleBuilder&MB=true&action=editLayout&view='. $this->editLayout . '&view_module=' . $this->editModule . '&view_package=' . $_REQUEST [ 'view_package' ] . '")' ) ;
 		} else
 		{
 			...
 			$ajax->addCrumb ( translate ( $searchLabel, 'ModuleBuilder' ), 'ModuleBuilder.getContent("module=ModuleBuilder&action=editLayout&view=' . $this->editLayout . '&view_module=' . $this->editModule . '")'  ) ;
 		}
 		return $ajax ;
 	}

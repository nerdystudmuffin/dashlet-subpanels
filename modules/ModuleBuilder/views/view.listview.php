...
class ViewListView extends ViewEdit
{
    ...
    function constructAjax ()
    {
        ...
        if ($this->fromModuleBuilder)
        {
            ...
            {
                $ajax->addCrumb ( $translatedViewType, 'ModuleBuilder.getContent("module=ModuleBuilder&MB=true&action=editLayout&view='.$this->editLayout.'&view_module=' . $this->editModule . '&view_package=' . $_REQUEST [ 'view_package' ] . '")' ) ;
            }
        } else
        {
...
//        $imageHelp = SugarThemeRegistry::current()->getImage('help') ;
        ...
        $buttons [] = array ( 'id' => 'historyDefault' , 'name' => 'historyDefault',  'text' => translate ( 'LBL_RESTORE_DEFUALT' ) , 'actionScript' => $restoreAction ) ;

    ...
    function display ($preview = false)
    {
        ...
        } else
        {
            ...
            if (! $this->fromModuleBuilder)
            {
                ...
                $buttons [] = array ( 'id' => 'historyDefault' , 'text' => translate ( 'LBL_RESTORE_DEFUALT' ) , 'actionScript' => "onclick='ModuleBuilder.history.revert(\"{$this->editModule}\", \"{$this->editLayout}\", \"{$history->getLast()}\", \"\")'" ) ;
            } else
            {
        ...
        if ($this->fromModuleBuilder)
        {
            ...
            $ajax->addCrumb ( $translatedViewType, 'ModuleBuilder.getContent("module=ModuleBuilder&action=editLayout&view=' . $this->editLayout . '&view_module=' . $this->editModule . '&view_package=' . $this->package . '")' ) ;
        } else
        {

	    		...
	    		if (isset($_REQUEST['login']) && $_REQUEST['login'] =='1'){
	    			if ( $row['username'] == $_POST['user_name'] ){	
					    ...
					    $query2 = "UPDATE users_password_link SET deleted='1' where id='".$_REQUEST['guid']."'";
				   		...
				   		header('location:index.php?action=Authenticate&module=Users&user_name='.$_REQUEST['user_name'].'&user_password='.$_REQUEST['new_password']);
						exit ();
				   	}
	    		}
				else
...
///////////////////////////////////////////////////////////////////////////////
	...
	if(isset($admin->settings['captcha_on'])&& $admin->settings['captcha_on']=='1' && !empty($admin->settings['captcha_private_key']) && !empty($admin->settings['captcha_public_key'])){
		...
		$captcha_js .="<script type='text/javascript' src='" . getJSPath('include/javascript/sugar_grp1_yui.js') . "'></script><script type='text/javascript' src='" . getJSPath('include/javascript/sugar_grp_yui2.js') . "'></script><script type='text/javascript' src='" . getJSPath('include/javascript/sugar_grp_yui_ext.js') . "'></script>
		...
		var handleSuccess = function(o){
			if(o.responseText!==undefined && o.responseText =='Success'){
				...
				alert(SUGAR.language.get('Users','LBL_FILL_CAPTCHA_FIELD'));
				Recaptcha.reload();
...
$sugar_smarty->assign('SUBMIT_BUTTON','<input title="'.$mod_strings['LBL_LOGIN_BUTTON_TITLE'].'" accessKey="'.$mod_strings['LBL_LOGIN_BUTTON_TITLE'].'" class="button" onclick="if(!set_password(form,newrules(\''.$GLOBALS["sugar_config"]["passwordsetting"]["minpwdlength"].'\',\''.$GLOBALS['sugar_config']['passwordsetting']['maxpwdlength'].'\',\''.$GLOBALS['sugar_config']['passwordsetting']['customregex'].'\'))) return false; validateCaptchaAndSubmit();" type="button" tabindex="3" id="login_button" name="Login" value="'.$mod_strings['LBL_LOGIN_BUTTON_LABEL'].'" ><br>&nbsp');

...
class UserDemoData {
	...
	*/
	...
	function create_demo_data() {
		// Create the VP
		if(!$this->_user->retrieve($this->guids['jim'])) {
			$this->_create_seed_user($this->guids['jim'], "Brennan", "Jim", "jim",
				"VP Sales", false, null, null, "jim@example.com");
		}
	
		// Create the west team
		if(!$this->_user->retrieve($this->guids['sarah'])) {
			$this->_create_seed_user($this->guids['sarah'], "Smith", "Sarah", "sarah",
				"Sales Manager West", false, $this->guids['jim'], "Brennan, Jim", "sarah@example.com");
		}
		if(!$this->_user->retrieve($this->guids['sally'])) {
			$this->_create_seed_user($this->guids['sally'], "Bronsen", "Sally", "sally",
				"Senior Account Rep", false, $this->guids['sarah'], "Smith, Sarah", "sally@example.com");
		}
		if(!$this->_user->retrieve($this->guids['max'])) {
			$this->_create_seed_user($this->guids['max'], "Jensen", "Max", "max",
				"Account Rep", false, $this->guids['sarah'], "Smith, Sarah", "max@example.com");
		}
	
		// Create the east team
		if(!$this->_user->retrieve($this->guids['will'])) {
			$this->_create_seed_user($this->guids['will'], "Westin", "Will", "will",
				"Sales Manager East", false, $this->guids['jim'], "Brennan, Jim", "will@example.com");
		}
		if(!$this->_user->retrieve($this->guids['chris'])) {
			$this->_create_seed_user($this->guids['chris'], "Olliver", "Chris", "chris",
				"Senior Account Rep", false, $this->guids['will'], "Westin, Will", "chris@example.com");
		}
	...
	function create_demo_data_jp() {
		// Create the VP
		if(!$this->_user->retrieve($this->guids['jim'])) {
			$this->_create_seed_user($this->guids['jim'], "ブレナン", "ジーム", "jim",
				"VP Sales", false, null, null, "jim@example.com");
		}
	
		// Create the west team
		if(!$this->_user->retrieve($this->guids['sarah'])) {
			$this->_create_seed_user($this->guids['sarah'], "スミス", "サーラー", "sarah",
				"Sales Manager West", false, $this->guids['jim'], "ブレナン, ジーム", "sarah@example.com");
		}
		if(!$this->_user->retrieve($this->guids['sally'])) {
			$this->_create_seed_user($this->guids['sally'], "ブロンソン", "サーリー", "sally",
				"Senior Account Rep", false, $this->guids['sarah'], "スミス, サーラー", "sally@example.com");
		}
		if(!$this->_user->retrieve($this->guids['max'])) {
			$this->_create_seed_user($this->guids['max'], "ジェンソン", "マクス", "max",
				"Account Rep", false, $this->guids['sarah'], "スミス、サーラー", "max@example.com");
		}
	
		// Create the east team
		if(!$this->_user->retrieve($this->guids['will'])) {
			$this->_create_seed_user($this->guids['will'], "ウエストン", "ウイル", "will",
				"Sales Manager East", false, $this->guids['jim'], "ブレナン, ジーム", "will@example.com");
		}
		if(!$this->_user->retrieve($this->guids['chris'])) {
			$this->_create_seed_user($this->guids['chris'], "オリバー", "クリス", "chris",
				"Senior Account Rep", false, $this->guids['will'], "ウエストン, ウイル", "chris@example.com");
		}
	}
	...
	function _quick_create_user($name)
	{
		if (!$this->_user->retrieve($name.'_id'))
		{
			$this->_create_seed_user("{$name}_id", $name, $name, $name,
				"Sales Manager of no territory", false, "seed_jim_id", "Brennan, Jim", "jim@example.com");
		}
	}

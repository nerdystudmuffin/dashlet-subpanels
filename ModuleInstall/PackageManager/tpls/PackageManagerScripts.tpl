...
*}
{literal}
<link rel='stylesheet' type='text/css' href='include/javascript/sugar_grp_yui_widgets.css'/>
...
{/literal}{if $module_load == 'true'}{literal}
		...
		var patch_downloads_minWidth = moduleTitleEl.clientWidth / 9.6;
		_fileGrid = new YAHOO.widget.ScrollingDataTable(
                'patch_downloads',
                [
					{key:'name', label: '{/literal}{$ML_FILEGRID_COLUMN.Name}{literal}', minWidth: patch_downloads_minWidth, sortable: true, resizeable: true},
                    {key:'file', label: '{/literal}{$ML_FILEGRID_COLUMN.Install}{literal}', minWidth: patch_downloads_minWidth, formatter: this.renderInstallButton, resizeable: true},
                    {key:'unFile', label: '{/literal}{$ML_FILEGRID_COLUMN.Delete}{literal}', minWidth: patch_downloads_minWidth, formatter: this.renderDeleteButton, resizeable: true},
        		    {key:'type', label: '{/literal}{$ML_FILEGRID_COLUMN.Type}{literal}', minWidth: patch_downloads_minWidth},
        		    {key:'version', label: '{/literal}{$ML_FILEGRID_COLUMN.Version}{literal}', minWidth: patch_downloads_minWidth},
        		    {key:'date', label: '{/literal}{$ML_FILEGRID_COLUMN.Published}{literal}', minWidth: patch_downloads_minWidth},
        		    {key:'uninstallable',  label: '{/literal}{$ML_FILEGRID_COLUMN.Uninstallable}{literal}', minWidth: patch_downloads_minWidth},
        		    {key:'description',label: '{/literal}{$ML_FILEGRID_COLUMN.Description}{literal}', minWidth: patch_downloads_minWidth, sortable: true}
        		],
                ...
                {MSG_EMPTY: "", width : patch_downloads_tableWidth}
            );
        _fileGrid.autoSizeColumns = true;
        _fileGrid.MSG_EMPTY = "empty4";
        _fileGrid.subscribe("postRenderEvent", function() {
        		if (mti_data.length <= 0) {
        			 var bodyDiv = _fileGrid.getBdContainerEl();
        			 var bodyTable = _fileGrid.getBdTableEl();
        			 bodyTable.style.width = bodyDiv.style.width;
        			 
        			 var headerDiv = _fileGrid.getHdContainerEl();
        			 var headerTable = _fileGrid.getHdTableEl();
        			 headerTable.style.width = headerDiv.style.width;
        		}
        	}
        );
        ...
        var moduleTitleEl2 = YAHOO.util.Dom.getElementsByClassName("moduleTitle yui-skin-sam")[0];
        var installed_grid_tableWidth = moduleTitleEl2.clientWidth - 2+ "px";
		var minWidth = moduleTitleEl2.clientWidth / 8.2;
        _fileGridInstalled = new YAHOO.widget.ScrollingDataTable('installed_grid',
            [
			    {key:'name', label: '{/literal}{$ML_FILEGRIDINSTALLED_COLUMN.Name}{literal}', sortable: true, 'minWidth' : minWidth},
                
                {key:'unFile', label: '{/literal}{$ML_FILEGRIDINSTALLED_COLUMN.Action}{literal}', formatter: this.renderUninstallButton, 'minWidth' : minWidth},
                {key:'state_file', label: '{/literal}{$ML_FILEGRIDINSTALLED_COLUMN.Enable_Or_Disable}{literal}',formatter: this.renderEnableDisableButton, 'minWidth' : minWidth},
      		    {key:'type', label: '{/literal}{$ML_FILEGRIDINSTALLED_COLUMN.Type}{literal}', 'minWidth' : minWidth},
      		    {key:'version', label: '{/literal}{$ML_FILEGRIDINSTALLED_COLUMN.Version}{literal}', 'minWidth' : minWidth},
      		    {key:'date', label: '{/literal}{$ML_FILEGRIDINSTALLED_COLUMN.Date_Installed}{literal}', 'minWidth' : minWidth},
      		   
      		    {key:'description', label: '{/literal}{$ML_FILEGRIDINSTALLED_COLUMN.Description}{literal}', sortable: true, 'minWidth' : minWidth}
      		],
      		new YAHOO.util.LocalDataSource(mti_installed_data, {
          	    ...
          	    height: "200px"}), 
            {MSG_EMPTY: "", width : installed_grid_tableWidth});

         _fileGridInstalled.autoSizeColumns = true;
         _fileGridInstalled.MSG_EMPTY = "empty5";
        _fileGridInstalled.subscribe("postRenderEvent", function() {
        		if (mti_installed_data.length <= 0) {
        			 var bodyDiv = _fileGridInstalled.getBdContainerEl();
        			 var bodyTable = _fileGridInstalled.getBdTableEl();
        			 bodyTable.style.width = bodyDiv.style.width;
        			 
        			 var headerDiv = _fileGridInstalled.getHdContainerEl();
        			 var headerTable = _fileGridInstalled.getHdTableEl();
        			 headerTable.style.width = headerDiv.style.width;
        		}
        	}
        );
        _fileGridInstalled.subscribe("beforeWidthChange", function() {
        		if (mti_installed_data.length <= 0) {
        			return false;
        		}
        	}

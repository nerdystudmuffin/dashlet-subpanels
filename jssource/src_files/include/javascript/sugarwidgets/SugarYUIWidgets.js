		...
		if (config['type'] && config['type'] == "prompt") {
			myConf['buttons'] = [{
				...
				text: SUGAR.language.get("app_strings", "LBL_EMAIL_OK"), handler:config['fn'] ? 
								function(){YAHOO.SUGAR.MessageBox.hide(); config['fn'](YAHOO.util.Dom.get("sugar-message-prompt").value);} 
...
sw.Template.prototype = {
	...
	append: function (target, args) {
		...
		else console.log("Warning, unable to find target:" + target);
	},
	exec : function (args) {
...
YAHOO.extend(sw.AsyncPanel, YAHOO.widget.Panel, {
	...
	_updateContent : function (o) {
		//Under safari, the width of the panel may expand for no apparent reason
